...
## Introduction
In this project, a Transfer Learning(TL) model is deployed on the device used to detect whether the trash can in the kitchen is full or not. A camera installed on the device continuously takes pictures of the inside of the trash can and provides data visualization via an LED Strip. The overall appearance of the project is shown below：
